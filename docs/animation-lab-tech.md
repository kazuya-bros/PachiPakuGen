...
軽量フォールバックとしてB1（レイヤー全体の遅延バネ）を残す設計も可。
...
| [PuruPuruPNGTuber](https://github.com/rotejin/PuruPuruPNGTuber)（ろてじん氏） | 前髪/後ろ髪の遅延追従揺れ、顔向き調整、目元演出（ハイライト・涙レンズ・影）、自動瞬き。ウェーブ式の技術調査で参照 | **Apache-2.0**（帰属表示＋変更明示でコード参照・流用可） |
...
**口＝A1モーラ同期、首・視線・房物理＝パララックス＋房別スプリングリグ**を組み合わせる。
...
| 3 | **胸部追従**（§8.5。局所変形の考え方を参考） | 追加素材不要。任意の`chest.png`は位置ガイド兼旧形式互換 | 強減衰バネ＋body/衣服のガウス局所ワープ | ★★★ |
...
- `depth`: パララックス係数（-1..1、0=基準面。参考実装の名前ベース固定
...
### 8.5 #3 胸部追従の実装仕様（PachiPakuGen向け局所ワープ）

```
素材: body.png。chest.pngがあれば変形範囲の位置ガイドに使うが、重ね描きはしない。
      無い場合はbodyの不透明範囲から上半身中央を推定する。
物理: 縦バネ1本（肩の弾みliftより小さく、強減衰）
  駆動:
   - 体の上下動のY速度カップリング（呼吸で常時わずかに上下）
   - 発話開始・登場時の撃力（肩の弾みより遅れて・小さく = 二次揺れ）
   - 息継ぎ（§8.6）で吸気の持ち上がりに連動
 変形: body/衣服の胸周辺だけをガウス重み付きでY方向にラスターワープする。
       3σ外は変位をほぼ0にし、輪郭・影の二重表示を避ける。
 既定値: k=45 / c=12 / 最大3.5px。初期状態は無効。
留意: Anime2.5DRigのメッシュ変形をそのまま移植したものではなく、画像レイヤー向けの独自実装。
      表現強度は好みが分かれるため、設定UIで 0（無効）〜強 のスライダーを維持する。
      感情別倍率（§8.6）の対象に含める
```
...
1. **アプリ内ライブ表示** ★推奨
   素材・プロファイル・アニメーションランタイムをアプリ内で直接再生し、
   マイク音量で口を開閉する。OBSではPachiPakuGenの表示領域を
   ウィンドウキャプチャして、単体PNGTuberとして利用できる
2. **SpriTalk形式の設定出力**
   母音口パクやモーション設定が必要な場合は、従来どおりSpriTalk向けの
   プロファイルを書き出す。ライブ表示と同じ調整値を利用する
