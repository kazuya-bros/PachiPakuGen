...
| `unet_segmentation.onnx` | 6MB | UNet セグメンテーション | アプリに同梱 |
| `sam3.pt` | 3.5GB | SAM3 セグメンテーション（Python連携） | 別途DL |

開発時は `src-tauri/models/` に配置してください。
...
models/
  sam3.pt              <-- ここに配置
  rife.onnx            （自動バンドル済み）
  unet_segmentation.onnx （自動バンドル済み）
```
...
| `unet_segmentation.onnx` | [face-parsing.PyTorch](https://github.com/zllrunning/face-parsing.PyTorch) (BiSeNet) | MIT License |
